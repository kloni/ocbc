...
//			 add potential predecessor to successor list and finger table and
	...
	public void broadcast(Broadcast info) throws CommunicationException {
		if (this.logger.isEnabledFor(DEBUG)) {
			this.logger.debug(impl + ": Send broadcast message: " + info);
		}
		
		if(impl.getTransactionId()!=null) {
			int oldtid = impl.getTransactionId();
			int newtid = info.getTransaction();
			if(newtid>(oldtid+1)) {
				this.logger.warn("Missing transactions! Expected " + (oldtid+1) + ", got " + newtid);
			} else if(newtid<oldtid) {
				this.logger.warn("Redundant transaction! Expected " + (oldtid+1) + ", got " + newtid);
				//return;
			} else if(newtid==oldtid) {
				// my own tx
				return;
		...
		GraphTool.writePredecessor(info.getTransaction(), impl.getID(), impl.getPredecessorID());
		
		Node prev = null;
		for(Node node : impl.getFixedFingerTable()) {
			GraphTool.writeFinger(info.getTransaction(), impl.getID(), node.getNodeID());
			
			if(node.getNodeID().isInInterval(getNodeID(), info.getRange())) {
				if(prev!=null) {
					try {
						prev.broadcast(new Broadcast(node.getNodeID(), info.getSource(), info.getTarget(), info.getTransaction(), info.getHit()));
					} catch (CommunicationException e) {
						logger.error("Broadcast to " + prev + " failed! " + e.getMessage());
					}
				}
				prev = node;
			}
		...
		if(prev!=null) {
			try {
				prev.broadcast(new Broadcast(info.getRange(), info.getSource(), info.getTarget(), info.getTransaction(), info.getHit()));
			} catch (CommunicationException e) {
				logger.error("Broadcast to " + prev + " failed! " + e.getMessage());
			}
		}
